...
cv_tuples <- get_cross_validation_tuples_step1()
...
for (t in 1:TN) {
  ...
  {
    state_file <- sprintf("%s/%s/spmkl_step1_rep_%d_state.RData", result_path, cohort, replication)
    ...
    if (file.exists(state_file) == FALSE){
      ...
      train_ratio <- 0.8
      optimality_gap <- 1e-5
      phase_iterations <- c(100, 25, 75)
      phase1_method <- "Iterative"
      phase3_method <- "Iterative"
      ...
      auroc_cv_tuples <- matrix(0, nrow = nrow(cv_tuples), ncol = 2+fold_count)
      colnames(auroc_cv_tuples) <- c("C", "G", paste("Auroc",1:fold_count))
      ...
      for (fold in 1:fold_count) {
        ...
        for (tpl in 1:nrow(cv_tuples)) {
          C_tpl <- cv_tuples[tpl, "C"]
          G_tpl <- cv_tuples[tpl, "G"]
          
          print(sprintf("%s Replication %d --> fold = %d, C = %g, G = %g", cohort, replication, fold, C_tpl, G_tpl))
          parameters <- list()
          parameters$C <- C_tpl
          parameters$G <- G_tpl
          parameters$epsilon <- epsilon
          parameters$phase_iterations <- phase_iterations
          parameters$optimality_gap <- optimality_gap
          parameters$phase1_method <- phase1_method
          parameters$phase3_method <- phase3_method
          
          output <- spmkl_step1_three_phase(K_train, y_train, parameters)
          state <- output$state
          prediction <- spmkl_test(K_test, state)
          AUROC <- auc(roc(prediction$f, as.factor(y_test)))
          
          auroc_cv_tuples[tpl, c("C", "G", paste("Auroc", fold))] <- c(C_tpl, G_tpl, AUROC)
        }
      }
      ...
      average_aurocs <- rowMeans(auroc_cv_tuples[,2+(1:fold_count)])
      tuple_star <- which(average_aurocs == max(average_aurocs))[1]
      C_star <- auroc_cv_tuples[tuple_star, "C"]
      G_star <- auroc_cv_tuples[tuple_star, "G"]
      
      print(sprintf("%s Replication %d --> C = %g, G = %g", cohort, replication, C_star, G_star))
